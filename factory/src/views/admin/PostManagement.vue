      ...
      <table class="w-full text-sm" style="min-width: 1000px;">
        <thead class="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
          <tr>
            ...
            <th v-if="getColumnVisible('name')" class="px-5 py-4 text-center">岗位名称</th>
            <th v-if="getColumnVisible('order')" class="px-5 py-4 text-center">岗位排序</th>
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-else v-for="position in dataList" :key="position.id" class="hover:bg-slate-50/80 transition-colors group">
            ...
            <td v-if="getColumnVisible('index')" class="px-5 py-4 text-center text-slate-500">
              {{ (currentPage - 1) * pageSize + dataList.indexOf(position) + 1 }}
            </td>
            ...
            <td v-if="getColumnVisible('created_by')" class="px-5 py-4 text-center text-slate-600">
              {{ position.created_by?.name || '-' }}
            </td>
            <td v-if="getColumnVisible('updated_by')" class="px-5 py-4 text-center text-slate-600">
      ...
      <div v-if="dialogType === 'detail'" class="p-6">
        <div class="grid grid-cols-2 gap-5">
          <div class="p-4 bg-slate-50 rounded-xl">
            <div class="text-xs text-slate-500 mb-1 uppercase tracking-wide">岗位名称</div>
            <div class="font-semibold text-slate-900">{{ detailData.name }}</div>
          </div>
          <div class="p-4 bg-slate-50 rounded-xl">
            <div class="text-xs text-slate-500 mb-1 uppercase tracking-wide">岗位排序</div>
...
import type { PositionForm, PositionTable } from '@/services/fastApi/module_system/position'
import type { User } from '@/types/user'

// 权限配置
const PERMS = {
  query: 'module_system:position:query',
  create: 'module_system:position:create',
  update: 'module_system:position:update',
  delete: 'module_system:position:delete',
  export: 'module_system:position:export',
  patch: 'module_system:position:patch',
  detail: 'module_system:position:detail',
} as const
...
const {
  ...
  handleReset,
  handleRefresh,
} = useCrudPage<PositionTable, { name: string; status: string; start_time: string; end_time: string; created_id?: number }>({
  initialQuery: {
    name: '',
    status: '',
    start_time: '',
  ...
  fetchList: async (params) => {
    ...
    if (params.created_id) queryParamsExport.created_id = params.created_id
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    await api.position.deletePosition(selectedIds.value as number[])
    fetchData()
...
const handleBatchStatus = async (status: string) => {
  ...
  try {
    ...
    await api.position.batchPosition({ ids: selectedIds.value as number[], status })
    fetchData()
