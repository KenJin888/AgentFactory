<template>
  <div class="min-h-screen bg-slate-50 p-8 overflow-y-auto">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 class="text-3xl font-bold text-slate-900 flex items-center gap-3">
             <ShieldIcon class="text-blue-600" /> 角色管理
          </h1>
        </div>
              ...
              <tr v-else v-for="role in roles" :key="role.id" class="hover:bg-slate-50/80 transition-colors group">
                ...
                <td v-if="getColumnVisible('action')" class="px-5 py-4 text-center fixed-col">
                  <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      @click="handleManageDataset(role)"
                      class="p-2 text-slate-800 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"
                      title="知识库权限"
                      :disabled="role.id === 1"
                      :class="{ 'opacity-50 cursor-not-allowed': role.id === 1 }"
                    >
    ...
    <!-- Dataset Permission Modal -->
    <DatasetPermissionModal
      v-model:visible="datasetModalVisible"
      :role-id="currentRoleForDataset.id || 0"
      :role-name="currentRoleForDataset.name || ''"
      @confirm="handleDatasetConfirm"
      @cancel="handleDatasetCancel"
    />
...
import DatasetPermissionModal from '@/components/common/DatasetPermissionModal.vue';
...
import type { RoleTable, RoleForm } from '@/services/fastApi/module_system/role';
...
const currentRoleForDataset = ref<RoleTable>({} as RoleTable);
...
const handleManageDataset = (role: RoleTable) => {
  if (role.id === 1) {
    message.warning('系统默认角色，不可操作');
    return;
  }
  currentRoleForDataset.value = role;
  datasetModalVisible.value = true;
};

const handleDatasetConfirm = () => {
  // 提示已在 DatasetPermissionModal 中处理
};
