    ...
    >
      <div class="bg-white rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h3 class="text-lg font-bold text-slate-800">知识库权限设置</h3>
        ...
        <div class="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          <div
              :class="hasDefaultRule ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-blue-50 border-blue-200 text-blue-700'"
          ...
          <p v-if="authRules.length === 0" class="text-sm text-red-500">
            至少保留一条授权规则
          </p>
        </div>
        ...
        <div class="px-6 py-4 bg-slate-50 flex justify-end gap-3">
          ...
          <button
              :disabled="loading || saving || authRules.length === 0 || !datasetId"
...
import type {DatasetAuthRule} from '@/services/fastApi/module_ai/ai_ragflow'
...
const emit = defineEmits<{
  close: []
  saved: [rules: DatasetAuthRule[]]
}>()

type EditableDatasetAuthRule = DatasetAuthRule & {
...
const authRules = ref<EditableDatasetAuthRule[]>([])
const roleSelectorVisible = ref(false)
const userSelectorVisible = ref(false)
const roleSectionEnabled = ref(false)
const userSectionEnabled = ref(false)

const normalizeRules = (rules?: DatasetAuthRule[] | null): EditableDatasetAuthRule[] => {
  const dedup = new Map<string, EditableDatasetAuthRule>()
  for (const item of rules || []) {
    const targetType = item.target_type
    const targetValue = targetType === 'global' ? null : String(item.target_value || '')
    if (!targetType || (targetType !== 'global' && !targetValue)) continue

    const key = `${targetType}:${targetValue ?? 'global'}`
    const existed = dedup.get(key)
    const nextRule: EditableDatasetAuthRule = {
      ...item,
...
const updateRules = (updater: (rules: EditableDatasetAuthRule[]) => EditableDatasetAuthRule[]) => {
  authRules.value = normalizeRules(updater([...authRules.value]))
}

const hasDefaultRule = computed(() => !loading.value && Boolean(props.datasetId) && authRules.value.length === 0)
...
const noticeText = computed(() => (
    hasDefaultRule.value
        ? '当前未单独配置权限。管理员默认拥有全部权限，其他角色和用户默认无权限；保存后将改为显式权限配置。'
        : '保存后将按下面的规则控制知识库的只读或读写权限。'
))

const syncSectionStateFromRules = (rules: EditableDatasetAuthRule[]) => {
  ...
  updateRules((rules) => {
    ...
    const nextRoleRules = Array.from(selectedMap.values()).map((role) => {
      ...
      return {
        ...
        target_right: (existed?.target_right === 2 ? 2 : 1) as 1 | 2,
      }
    })
...
const loadRules = async () => {
  ...
  loading.value = true
  try {
    const res = await api.knowledge.listDatasetAuthRules(props.datasetId)
    authRules.value = normalizeRules(res.data || [])
...
const handleConfirm = async () => {
  ...
  saving.value = true
  try {
    const res = await api.knowledge.updateDatasetAuthRules(props.datasetId, {
...
watch(
    () => [props.visible, props.datasetId] as const,
    async ([visible]) => {
      if (!visible) {
        authRules.value = []
        roleSectionEnabled.value = false
        userSectionEnabled.value = false
