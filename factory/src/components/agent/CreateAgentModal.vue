<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
    <div class="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden border border-slate-100 flex flex-col h-[90vh]">
      <!-- Header -->
      <div :class="`px-8 py-6 border-b border-gray-100 flex justify-between items-center ${isAdminMode ? 'bg-slate-900 text-white' : 'bg-gray-50/50'}`">
        <div>
      ...
      <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
          <form id="agent-form" @submit.prevent="handleSubmit" class="space-y-6">
              <!-- 基础设定 -->
              ...
              <div v-if="activeTab === 'skills'" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div v-else-if="skillsLoadError" class="p-4 bg-red-50 text-red-600 rounded-xl text-sm">{{ skillsLoadError }}</div>
                  <div v-else-if="skillList.length === 0" class="p-4 bg-slate-50 text-slate-500 rounded-xl text-sm">暂无可选技能</div>
                  <div v-else class="max-h-[calc(90vh-22rem)] min-h-40 overflow-y-auto border border-slate-200 rounded-xl divide-y divide-slate-100">
                      <label
                      ...
                      >
                          ...
                          <div class="min-w-0">
                              ...
                              <div class="text-xs text-slate-500 whitespace-pre-wrap break-words">{{ skill.description || '未设置描述' }}</div>
                          </div>
                      </label>
                  </div>
              </div>
                ...
                <div class="border border-slate-200 rounded-xl overflow-hidden">
                  ...
                  >
                    <div class="flex items-center gap-2">
                      ...
                      <span v-if="selectedInternalTools.length > 0" class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                  ...
                  <div v-show="isInternalToolsExpanded" class="border-t border-slate-200">
                    ...
                    <div v-else-if="internalToolsLoadError" class="p-4 bg-red-50 text-red-600 text-sm">{{ internalToolsLoadError }}</div>
                    <div v-else-if="internalToolsList.length === 0" class="p-4 bg-slate-50 text-slate-500 text-sm">暂无内置工具</div>
                    <div v-else class="max-h-60 overflow-y-auto divide-y divide-slate-100">
                      ...
                      >
                        ...
                        <div class="min-w-0">
                          ...
                          <div class="text-xs text-slate-500 whitespace-pre-wrap break-words">{{ tool.description || '未设置描述' }}</div>
                        </div>
                      </label>
                    </div>
                  </div>
                ...
                <div class="border border-slate-200 rounded-xl overflow-hidden">
                  ...
                  >
                    <div class="flex items-center gap-2">
                      ...
                      <span v-if="selectedExternalTools.length > 0" class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                  ...
                  <div v-show="isMcpToolsExpanded" class="border-t border-slate-200">
                    ...
                    <div v-else-if="visibleMcpList.length === 0" class="p-4 bg-slate-50 text-slate-500 text-sm">暂无可选 MCP</div>
                    <div v-else class="max-h-60 overflow-y-auto divide-y divide-slate-100">
                      <label
              ...
              <div v-if="activeTab === 'knowledge'" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                <div class="p-3 rounded-xl bg-blue-50 text-blue-700 text-sm border border-blue-100">
                  ...
                  <div v-else-if="knowledgeLoadError" class="p-4 bg-red-50 text-red-600 rounded-xl text-sm">{{ knowledgeLoadError }}</div>
                  <div v-else-if="knowledgeList.length === 0" class="p-4 bg-slate-50 text-slate-500 rounded-xl text-sm">暂无可选知识库</div>
                  <div v-else class="max-h-[calc(90vh-22rem)] min-h-40 overflow-y-auto border border-slate-200 rounded-xl divide-y divide-slate-100">
                      <label
                      ...
                      >
                          ...
                          <div class="min-w-0">
                              ...
                              <div class="text-xs text-slate-500 whitespace-pre-wrap break-words">{{ dataset.description || '未设置描述' }}</div>
                          </div>
                      </label>
                  </div>
              </div>
      ...
      <div class="px-8 py-6 border-t border-gray-100 bg-gray-50/50 flex  gap-3">
            <button 
                ...
                @click="handleExport"
            >
                <Download :size="16" /> <span className="hidden sm:inline">导出配置</span>
            </button>
...
const normalizeMcpToolNames = (value: unknown): string[] => {
  if (!Array.isArray(value)) return []
  return value.filter((item): item is string => typeof item === 'string' && item.length > 0)
}
// 兼容旧数据：优先使用 externalTools，如果没有则尝试从 enabledMcpToolIds 迁移（通过名称匹配）
const initialExternalTools = normalizeMcpToolNames(initialConfig?.mcp?.externalTools)
