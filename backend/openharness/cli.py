...
http_app = typer.Typer(name="http", help="Run OpenHarness HTTP service")
...
def http_serve(
    host: str = typer.Option("127.0.0.1", "--host", help="Host for the HTTP server"),
    port: int = typer.Option(8787, "--port", help="Port for the HTTP server"),
    reload: bool = typer.Option(False, "--reload", help="Enable auto-reload for local development"),
    cwd: str = typer.Option(str(Path.cwd()), "--cwd", help="Working directory for session persistence"),
) -> None:
    """Start FastAPI-based OpenHarness HTTP service."""
    import uvicorn

    from app.plugin.module_oh.oh_http import create_http_app

    app_instance = create_http_app(cwd=cwd)
