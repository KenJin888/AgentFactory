...
async def get_obj_list_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:query"], check_data_scope=False))],
) -> JSONResponse:
    """
    查询用户
...
async def get_obj_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:detail"], check_data_scope=False))],
) -> JSONResponse:
    """
    查询用户详情
...
async def create_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:create"], check_data_scope=False))],
) -> JSONResponse:
    """
    创建用户
...
async def update_obj_controller(
    ...
    id: Annotated[int, Path(description="用户ID")],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:update"], check_data_scope=False))],
) -> JSONResponse:
    """
    修改用户
...
async def delete_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:delete"], check_data_scope=False))],
) -> JSONResponse:
    """
    删除用户
...
async def batch_set_available_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:patch"], check_data_scope=False))],
) -> JSONResponse:
    """
    批量修改用户状态
...
@UserRouter.post(
    ...
    dependencies=[Depends(AuthPermission(["module_system:user:download"], check_data_scope=False))],
)
...
async def export_obj_list_controller(
    page: Annotated[PaginationQueryParam, Depends()],
    search: Annotated[UserQueryParam, Depends()],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:export"], check_data_scope=False))],
) -> StreamingResponse:
    """
    导出用户
...
async def import_obj_list_controller(
    file: UploadFile,
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_system:user:import"], check_data_scope=False))],
) -> JSONResponse:
    """
    导入用户
