    ...
    async def __build_conditions(self, **kwargs) -> builtins.list[ColumnElement]:
        ...
        for key, value in kwargs.items():
            ...
            def add_condition(seq, val):
                ...
                elif seq == "not like" and val:
                    conditions.append(attr.not_like(val))
                elif seq == "in" and val:
            ...
            if isinstance(value, list) and len(value) > 0 and isinstance(value[0], tuple):
                for seq, val in value:
                    add_condition(seq, val)
            elif isinstance(value, tuple):
                add_condition(value[0], value[1])
            else:
