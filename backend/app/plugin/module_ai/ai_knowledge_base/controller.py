...
from .service import FilesService, get_file_dir
...
async def list_directory_controller(
    ...
    ai_file = FilesService(get_file_dir(conversation_id))
...
@AIFilesRouter.post(
    "/{conversation_id}/directory/create",
    summary="创建目录",
    description="创建新目录",
...
async def read_file_controller(
    ...
    ai_file = FilesService(get_file_dir(conversation_id))
    result = ai_file.read_file(req.path, url_state=True)
    log.info(f"读取文件成功: {req.path}")
    return SuccessResponse(data=result, msg="读取文件成功")


@AIFilesRouter.put(
    "/{conversation_id}/file/update",
    summary="更新文件内容",
