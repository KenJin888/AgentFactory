...
from fastmcp.server.http import create_base_app, create_sse_app
...
async def lifespan(app: FastAPI) -> AsyncGenerator[Any, Any]:
    ...
    from app.plugin.module_ai.ai_ragflow.service import close_ragflow_service_singleton
    ...
    try:
        try:
            from app.plugin.module_oh.oh_http.service import close_oh_session_manager_singleton

            await close_oh_session_manager_singleton()
        except Exception:
            pass
        await close_ragflow_service_singleton()
        log.info("✅ RAGFlow客户端已关闭")
        await import_modules_async(
...
def register_routers(app: FastAPI) -> None:
    ...
    app.include_router(common_router, dependencies=[Depends(RateLimiter(times=25, seconds=10))])
    app.include_router(system_router, dependencies=[Depends(RateLimiter(times=25, seconds=10))])
    app.include_router(monitor_router, dependencies=[Depends(RateLimiter(times=25, seconds=10))])
    ...
    app.include_router(
        ...
        # dependencies=[Depends(RateLimiter(times=25, seconds=10))],
    )
...
def register_mcp(app: FastAPI) -> None:
    def _mcp_route_map_fn(route: HTTPRoute, default_type: MCPType) -> MCPType:
        include_operations = set(settings.MCP_INCLUDE_OPERATIONS or [])
        if not include_operations:
            return default_type
        if route.operation_id == 'List datasets':
            pass
        return MCPType.TOOL if route.operation_id in settings.MCP_INCLUDE_OPERATIONS else MCPType.EXCLUDE
    ...
    mcp = FastMCP.from_fastapi(
        app=app,
        name=f"{settings.TITLE} MCP",
        route_map_fn=_mcp_route_map_fn,
    )
    ...
    mcp_app = create_base_app(
        routes=[*mcp_http_app.router.routes, *mcp_sse_app.router.routes],
        middleware=[],
